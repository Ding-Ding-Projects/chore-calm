...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| U01 | English, Hong Kong-style Cantonese, and bilingual language modes | Implemented in page settings and story copy; persistence is browser-local. | `docs/features/chore-coaching.md` | browser storage | c43 runtime interaction verified | `runtime-audit.json` | `settings-1440x1000.png` | Verified for listed live tuple |
| U02 | Emoji display control | Implemented as a browser-local message decoration preference; control text stays factual. | feature article | browser storage | c43 runtime interaction verified | `runtime-audit.json` | `settings-1440x1000.png` | Verified for listed live tuple |
| U03 | Personal-vocabulary JSON upload | Implemented as a bounded local JSON picker with clear/reset; private values must never enter public records. | README and feature article | browser storage only | component and source checks pass; file-flow evidence pending | `runtime-audit.json` | `settings-1440x1000.png` | Pending file-flow evidence |
| U04 | Funny-level controls for English and Cantonese | Implemented as two independent local sliders with localized tone output. | feature article | browser storage | c43 runtime interaction verified | `runtime-audit.json` | `settings-1440x1000.png` | Verified for listed live tuple |
...
| U09 | Non-blocking notifications | Implemented as in-page non-blocking status messages; system notifications are not claimed. | feature article | browser state | c43 playback and toast interaction verified | `runtime-audit.json` | `desktop-1440x1000.png` | Verified for listed live tuple |
...
| U14 | Command palette with `Ctrl+Shift+F` | Implemented as a keyboard-accessible local palette that focuses or scrolls to concept controls. | `docs/README.md` | browser state | c43 open, search, and result interaction verified | `runtime-audit.json` | `settings-1440x1000.png` | Verified for listed live tuple |
...
| U24 | Accessibility and responsive sizing | Implemented with keyboard paths, focus styles, text alternatives, reduced motion, and 320 px CSS targets; the listed built tuples have no overflow and no unnamed controls. | feature article | browser preference | c43 runtime and accessibility checks verified | `runtime-audit.json` | `desktop-1440x1000.png`, `mobile-390x844.png` | Verified for listed live tuples |
| U25 | Offline landing page and documentation | Root `index.html`, local scripts, local assets, and documentation are staged offline by the build route. | `docs/README.md` | none | strict local validation and live response verified | built output and run 34888216195 | `desktop-1440x1000.png` | Verified for listed live tuple |
...
| U28 | App logo and link-preview graphic | Product-specific mascot branding, `social-preview.png`, and static Open Graph metadata are implemented; live HTML and image fetch are verified. | ROADMAP.md | browser storage | local decode, strict validation, and live response pass | live Pages response | `desktop-1440x1000.png` | Verified for listed live tuple |
| U29 | Running version and updated-at provenance | The front screen reads version `0.1.0` and commit-bound updated-at provenance from `build-provenance.js`; the listed built capture records it. | `build-manifest.json` | manifest | strict local validation and c43 runtime pass | commit-bound local build and run 34888216195 | `desktop-1440x1000.png`, `settings-1440x1000.png`, `mobile-390x844.png` | Verified for listed live tuples |
...
| U32 | Overlays, keyboard shortcuts, progress, and recovery | Implemented with dialogs, keyboard palette, story progress, pause, recovery step, and non-blocking notifications. | feature article | browser state | c43 runtime interaction verified | `runtime-audit.json` | `desktop-1440x1000.png`, `settings-1440x1000.png` | Verified for listed live tuples |
...
| U37 | Clean-source build, manifest, dependency, and installer receipts | Root scripts and manifest are present; local build, strict validation, and Pages build job are green. | README and manifest | manifest | `scripts/validate-repository.ps1` | commit-bound local build and run 34888216195 | `runtime-audit.json` | Verified for listed build tuple |
